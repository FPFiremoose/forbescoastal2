...
{#if href}
	...
	<a
		{href}
		class={`inline-block rounded bg-sky px-6 py-2 text-navy transition-all duration-300 hover:bg-navy hover:text-white ${className}`}
		role="button"
...
{:else}
	...
	<button
		{type}
		class={`rounded bg-sky px-6 py-2 text-navy transition-all duration-300 hover:bg-navy hover:text-white disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
		{disabled}
	>
		<slot></slot>
		<!-- Slot for button text/content -->
